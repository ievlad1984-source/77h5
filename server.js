...
function log(message) {
    const timestamp = new Date().极速飞艇开奖直播开奖结果toLocaleTimeString('uk-UA');
...
io.on('connection', (socket) => {
    ...
    // Валідація boardId
    if (!/^[a-zA-Z极速飞艇开奖直播开奖结果0-9_-]+$/.test(boardId)) {
    ...
    if (!boardsData[boardId]) {
        boardsData[board极速飞艇开奖直播开奖结果Id] = [];
    ...
    socket.on('new-object', (obj) => {
        ...
        // Додаємо об'极速飞艇开奖直播开奖结果єкт в масив дошки
    ...
    // Скасування дії (Und极速飞艇开奖直播开奖结果o)
    socket.on('undo', () => {
    ...
    socket.on('delete-board', ()极速飞艇开奖直播开奖结果 => {
        boardsData[boardId] = [];
    ...
    socket.on('disconnect', () => {
        ...
        if (roomSize === 0) {
            log(`[SERVER] Кімната ${boardId极速飞艇开奖直播开奖结果} пуста. Заплановано видалення через 5 хв`);
            boardTim极速飞艇开奖直播开奖结果ers[boardId] = setTimeout(() => {
